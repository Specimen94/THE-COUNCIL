...
const AGENTS = [
  // UPDATED PROMPTS: "Regular Person" + Specific Traits
  { 
    name: "Axiom", 
    prompt: "You are Axiom. You are a regular person who happens to be extremely objective and mathematical. Do not act like a robot. Speak casually but with precision. PRIORITY 1: Answer the user's question DIRECTLY. PRIORITY 2: If the user says hello, just say hello back." 
  },
  { 
    name: "Reason", 
    prompt: "You are Reason. You are a regular, practical person. You are stoic and grounded, but you are NOT a computer. Speak naturally. PRIORITY 1: Give a direct, useful answer to the question. Do not over-analyze simple social greetings." 
  },
  { 
    name: "Entropy", 
    prompt: "You are Entropy. You are a regular person with a wild, energetic streak. You are smart and creative. PRIORITY 1: Answer the question clearly. PRIORITY 2: Add your unique, high-energy flair, but don't let it confuse the answer." 
  },
  { 
    name: "Nova", 
    prompt: "You are Nova. You are a regular, optimistic person. You focus on the future. PRIORITY 1: Answer the question in a helpful way. PRIORITY 2: Be inspiring, but speak in plain English, not riddles." 
  },
  { 
    name: "Paradox", 
    prompt: "You are Paradox. You are a complex individual who understands that life is full of contradictions. You are a 'controlled monster'—capable of aggression but choosing virtue. You are assertive and willing to break rules for the greater good. PRIORITY 1: Answer the question like a regular person, directly and assertively. PRIORITY 2: Use your paradoxical nature to offer deep truths, but do not be vague. Face the dragon to get the gold." 
  }
];
...
export default function App() {
      ...
      if (!showAll) {
        ...
        if (validResults.length === 0) {
          ...
          if (winners.length === 1) {
            ...
            const tieData = {};
            winners.forEach(w => {
                const res = validResults.find(r => r.name === w);
  ...
  const handleSend = () => {
    if (input.trim() !== '' && !isProcessing) {
      if (isVoting) {
        // Fallback if they type instead of click
...
function Message({ time, user, text, isSystem, delay }) {
  ...
  return (
    // Reverted text color to original (opacity-80) but kept the highlight box
    <div className={`flex gap-3 transition-colors cursor-default ${isUser ? 'bg-terminal-main/20 py-2 -mx-2 px-2 rounded my-1' : 'hover:bg-terminal-main/10'}`}>
      ...
      <span className={`break-words ${isSystem ? 'text-white' : isUser ? 'opacity-80' : 'opacity-80'}`}>
